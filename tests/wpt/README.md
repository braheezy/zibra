...
`testharness` runs each selected file in a real headless browser session. A
...
also supported. The current reviewed manifest contains only the legacy
`probe`, so `--mode testharness` selects no upstream cases until a genuinely
supported test is reviewed and added.
...
The manifest is the compatibility allowlist. Add tests only with a short
reason and an explicit mode; keep unsupported tests recorded as `skip` rather
than silently broadening the run.
