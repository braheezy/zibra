...
- `Node` children are stored by value in resizable arrays. Do not retain a
  ...
  synchronous relocation callback rebinds verified retained layout children
  before control can escape the mutation.
...
- Before general structural mutation destroys or relocates style fields, clear
  ...
  rebuilds live dependency edges afterward. A verified insertion-only mutation
