...
test "audio loading applies the Frame CSP intersection and media override" {
    const h = try Harness.init();
    defer h.deinit();
    const frame = h.tab.root_frame.?;
    try frame.applyContentSecurityPolicy("media-src data:, media-src 'none'", frame.current_url.?.*);
    try h.setSource();
    try h.eval("a.load(); a.error !== null && a.error.code === 4");
    try std.testing.expectEqual(@as(usize, 0), h.session.media_jobs.load(.acquire));
    try frame.applyContentSecurityPolicy("default-src 'none'; media-src data:", frame.current_url.?.*);
    try h.eval("a.load(); true");
    try h.deliverLoads();
    try h.eval("a.error === null && a.readyState === 4");
}
