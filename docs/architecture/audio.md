...
Each load snapshots owned target/referrer URLs, Referrer-Policy, an independently owned CSP
policy and protected origin, cancellation token and `(window_id, document_generation,
...
loads. It uses the shared Frame CSP parser and source matcher, including
intersected response policies, first-directive precedence, and a protected origin
independent of later base-URL changes. Each job copies that policy for initial
and redirect checks; redirected targets skip path matching while retaining
scheme, host and port restrictions.
