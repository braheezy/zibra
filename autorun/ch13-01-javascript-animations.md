...
- [ ] Implement a native `style_set` handler in `src/js.zig` that looks up the node by handle and updates its `style` attribute string.
- [ ] Mark the active tab as needing a render when `style_set` runs (schedule an animation frame if needed).
